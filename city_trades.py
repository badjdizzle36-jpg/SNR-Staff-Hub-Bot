...
function render(){
  ...
  // Centre against the active element's real layout position. This avoids
  // cumulative drift from responsive widths, filtered cards and scaling.
  const activeCentre=active.offsetLeft+(active.offsetWidth/2);
  deck.style.transform=`translate3d(${carousel.clientWidth/2-activeCentre}px,0,0)`;
...
carousel.addEventListener('keydown',event=>{if(event.key==='ArrowLeft'){event.preventDefault();move(-1)}else if(event.key==='ArrowRight'){event.preventDefault();move(1)}});
deck.addEventListener('pointerdown',event=>{startX=event.clientX;deck.setPointerCapture?.(event.pointerId)});
...
def exchange_html(store, owner, token):
    ...
    # Browse is a collection-led view: a player's available duplicate cards
    # must remain visible even when nobody has published an offer yet.
    cards = []
    for piece in data['pieces']:
        if piece['spares'] <= 0:
            continue
        tradeable = any(q['key'] != piece['key'] and q['rarity'] == piece['rarity'] for q in data['pieces'])
        if data['active'] and tradeable:
            action = (f'<button class="market-btn primary list-card" type="button" '
                      f'data-list-key="{esc(piece["key"])}">Offer this card</button>')
        elif not data['active']:
            action = '<button class="market-btn" type="button" disabled>Marketplace currently paused</button>'
        else:
            action = '<button class="market-btn" type="button" disabled>No same-rarity match available</button>'
        cards.append(
            f'<article class="offer-slide" data-rarity="{esc(piece["rarity"])}" data-kind="spare">'
            f'<div class="offer-owner"><span>YOUR DUPLICATE</span><b>×{piece["spares"]} AVAILABLE</b></div>'
            f'{market_card(piece["key"], piece["spares"], "YOUR COLLECTION")}'
            f'<p class="card-help">Your original stays protected. List one spare for a same-rarity sticker.</p>{action}</article>')
    ...
    for offer in data['offers']:
        ...
        cards.append(
            f'<article class="offer-slide" data-rarity="{esc(rarity)}" data-kind="offer">'
    ...
    empty_offers = '<div class="empty-state"><b>No cards in this filter.</b><p>Your available duplicates and live Marketplace offers will appear here.</p></div>'
    styles = '''<style>
...
.offer-carousel{position:relative;overflow:hidden;max-width:480px;margin:0 auto;user-select:none}.offer-carousel[hidden]{display:none}.offer-deck{display:flex;align-items:flex-start;transition:transform .38s cubic-bezier(.2,.8,.2,1);touch-action:pan-y;will-change:transform}.offer-slide{flex:0 0 min(78%,370px);padding:8px;opacity:.42;transform:scale(.9);transform-origin:center;transition:.38s}.offer-slide.active{opacity:1;transform:scale(1)}.offer-owner{display:flex;justify-content:space-between;gap:10px;margin:0 8px 8px;color:#e5c66e;font-size:11px;font-weight:900;letter-spacing:1px}.card-help{text-align:center;min-height:34px;margin:12px 10px;color:#aaa69f;font-size:11px;line-height:1.45}
...
<div class="market-bottom"><a href="/account#city-run">My board</a><a href="/city-run-trades">Refresh Marketplace</a></div></main>'''
    script = '<script src="/city-marketplace.js?v=4" defer></script>'
