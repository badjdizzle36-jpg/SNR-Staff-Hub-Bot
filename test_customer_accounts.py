...
class AccountTests(unittest.TestCase):
    ...
    def test_full_web_account_flow_and_phone_cookie(self):
        ...
        try:
            ...
            self.assertIn("Neon Street Food", public)
            ...
            self.assertIn("City Run is being prepared", body)
            self.assertIn('aria-valuemax="38"', body)
            self.assertIn('data-tab-target="home"', body)
            self.assertIn('data-tab-target="order"', body)
            self.assertIn('data-tab-target="city-run"', body)
            ...
            self.assertIn('aria-label="Your membership credit card"', body)
            self.assertIn('class="card-balance"', body)
            self.assertRegex(body, r'class="card-balance"><strong>\d+</strong><span>LOYALTY POINTS</span>')
            self.assertIn('🛍️ Click &amp; Collect', body)
            self.assertIn('🛵 Delivery', body)
            self.assertNotIn('Request Pack — 4 Points', body)
            self.assertNotIn('Trading Card Reward', body)
            self.assertIn('All membership cards', body)
            self.assertIn('SNR VIP premium membership card', body)
            self.assertRegex(body, r'class="card-number"[^>]*>••••&nbsp; ••••&nbsp; ••••&nbsp; \d{4}</div>')
            self.assertIn('class="card-network"', body)
            self.assertIn('SNR Champions', body)
            self.assertIn('Monthly Giveaway', body)
            self.assertIn('Confirmed purchases only', body)
            ...
            response, result = open_request("/claim", {"claim_request_key": "retired"}, cookie=session_cookie)
            self.assertEqual(response.status, 400)
            self.assertIn("packs have retired", result)
            self.assertEqual(self.db.get_customer("Cody Ortega")["loyalty_points"], 4)
            self.assertEqual(len(claims.pending()), 0)
        finally:
            server.shutdown(); server.server_close()
