    ...
    return page(f'{customer["display_name"]} • SNR Loyalty', f'''<section class="card quick-hub" id="customer-app">{review_popup}
      ...
      <div class="app-view active" data-app-view="home" role="tabpanel">{recovery}
        <section class="store-card unified-card {'vip-black' if membership['name'] == 'SNR VIP' else ''}" aria-label="Your membership card">
          ...
          <div class="member-actions"><button type="button" data-tab-target="order" data-instore="true"><span>Scan In Store</span><small>Order at the counter</small></button><button type="button" data-tab-target="order" data-neworder="true"><span>Start New Order</span><small>Delivery or pickup</small></button></div>
        </section>
...
tabs.forEach(t=>t.addEventListener("click",()=>{show(t.dataset.tabTarget);if(t.dataset.instore){const m=document.getElementById("fulfillment-type");if(m){m.value="instore";m.dispatchEvent(new Event("change"))}}if(t.dataset.neworder){const m=document.getElementById("fulfillment-type");if(m){m.value=[...m.options].some(o=>o.value==="delivery"&&!o.disabled)?"delivery":"pickup";m.dispatchEvent(new Event("change"))}}}));const wanted=location.hash.slice(1);if(names.has(wanted))show(wanted);
