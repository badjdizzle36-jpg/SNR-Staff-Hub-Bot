...
@media(max-width:560px){.review-box form{display:flex;flex-direction:column}.review-box button{width:100%}}
"""
    ...
    for row in rows:
        ...
        if row["status"] == "paid" and row.get("assigned_driver_id"):
            ...
            if review:
                ...
                review_html = f'''<div class="review-box"><strong>Rate your {subject}</strong><br><small>Your rating goes automatically to {html.escape(row["assigned_driver_name"])}.</small><form method="post" action="/review"><input type="hidden" name="review_request_key" value="{html.escape(form_token, quote=True)}"><input type="hidden" name="order_id" value="{int(row["id"])}"><select name="rating" required aria-label="Star rating"><option value="" disabled selected>Choose 1–5 stars</option><option value="5">★★★★★ — Excellent</option><option value="4">★★★★☆ — Very good</option><option value="3">★★★☆☆ — Good</option><option value="2">★★☆☆☆ — Could improve</option><option value="1">★☆☆☆☆ — Poor</option></select><input name="comment" maxlength="250" placeholder="Optional short review" aria-label="Optional review"><button type="submit">Send Rating</button></form></div>'''
