...
def delivery_section(customer: dict, orders: DeliveryStore, shifts: StaffShifts, form_token: str) -> str:
    ...
    if active:
        ...
        if orders.configured():
            ...
            mode_options += '<option value="instore">💳 In Store — pay at counter</option>'
    ...
    return f'''<section class="app-page delivery" id="delivery"><h2 class="app-page-title">🍔 Order Food — In Store, Delivery or Pickup</h2><div class="drawer-body"><p class="muted">Choose your deals and quantities. In-store orders go straight to staff for payment confirmation.</p>{order_form}<details class="compact-info"><summary>Previous orders</summary><div class="history">{recent or '<p class="muted">No orders yet.</p>'}</div></details></div></section>'''
...
def customer_page(customer: dict, claims: ClaimStore, orders: DeliveryStore, shifts: StaffShifts,
    ...
    return page(f'{customer["display_name"]} • SNR Loyalty', f'''<section class="card quick-hub" id="customer-app">{review_popup}
      ...
      <div class="app-view active" data-app-view="home" role="tabpanel">{recovery}
        <section class="quick-loyalty"><div class="quick-loyalty-top"><span>⭐ LOYALTY CARD</span><strong>{points} / 4 POINTS</strong></div><h2>{reward_title}</h2><p>{'Claim it now from Rewards.' if points >= 4 else 'Every qualifying meal moves you closer.'}</p><div class="quick-progress"><span style="width:{progress * 25}%"></span></div></section>
        <div class="quick-actions"><button type="button" class="store-card" data-tab-target="order" data-instore="true"><span class="store-card-top">SNR BUNS • DIGITAL MEMBER CARD <b>✦</b></span><strong>Scan Card In Store</strong><span>At the counter? Tap to choose your meals.</span><small>Choose your order · Pay staff · Earn rewards</small><span class="store-card-bottom">{html.escape(customer["display_name"])} <b>OPEN MY MENU →</b></span></button><button type="button" class="quick-action quick-action-main" data-tab-target="order" aria-selected="false"><span>🍔</span><strong>Start New Order</strong><small>Delivery or pickup</small></button><button type="button" class="quick-action" data-tab-target="raffle" aria-selected="false"><span>🎟️</span><strong>Live Raffle</strong><small>Choose numbers</small></button><button type="button" class="quick-action" data-tab-target="rewards" aria-selected="false"><span>🎁</span><strong>My Rewards</strong><small>View and claim</small></button></div>
...
tabs.forEach(t=>t.addEventListener("click",()=>{show(t.dataset.tabTarget);if(t.dataset.instore){const m=document.getElementById("fulfillment-type");if(m){m.value="instore";m.dispatchEvent(new Event("change"))}}}));const wanted=location.hash.slice(1);if(names.has(wanted))show(wanted);
