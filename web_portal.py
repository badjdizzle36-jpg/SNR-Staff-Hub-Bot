...
CITY_STICKER_IMAGES = {
    row["key"]: Path(__file__).with_name(f"city-sticker-{index + 1:02d}.jpg")
    for index, row in enumerate(BUSINESSES)
}
...
def city_run_section(customer: dict, city_run: CityRunStore, reveal_token: str) -> str:
    ...
    return f'''<section class="app-page city-board-page"><h2 class="app-page-title">🏁 SNR City Run</h2><div class="drawer-body"><div class="city-run-hero"><small>{status}</small><strong>Collect the businesses.<br>Win the ride.</strong><span>Qualifying meals award City Run stickers. Duplicates can appear, so completing all 38 takes commitment.</span></div><div class="city-run-score"><div><b>{int(board['unique_collected'])}</b><small>OF 38 COLLECTED</small></div><div><b>{available}</b><small>STICKERS READY</small></div><div><b>{int(board['duplicates'])}</b><small>DUPLICATES</small></div></div>{action}{board_markup}<section class="city-corners"><h3>🧭 Board corners</h3>{corners}</section><div class="city-grand"><strong>🏎️ Complete the City</strong>{grand_action}</div><details class="city-board-details"><summary>View route rewards and live collection details</summary><div class="city-board-route-list">{''.join(routes)}</div></details></div></section>'''
