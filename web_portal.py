...
CITY_RUN_BOARD_IMAGE = Path(__file__).with_name("city-run-board-v4.png").read_bytes()
...
.city-collection-focus{margin:14px 0 10px;padding:16px;border:2px solid #f4ce64;border-radius:17px;background:linear-gradient(135deg,#291b0a,#0d1118 65%,#09262b);box-shadow:0 0 22px #e3a72b2b}.city-collection-focus header{display:flex;align-items:end;justify-content:space-between;gap:12px}.city-collection-focus small,.city-collection-focus strong{display:block}.city-collection-focus header>span>small{color:#f4ce64;font-weight:950;letter-spacing:1.5px}.city-collection-focus header>span>strong{margin-top:3px;font-size:24px}.city-collection-focus header>b{color:#fff2bb;font-size:24px}.city-collection-track{height:12px;margin-top:12px;overflow:hidden;border:1px solid #f4ce6455;border-radius:99px;background:#050608}.city-collection-track span{display:block;height:100%;border-radius:99px;background:linear-gradient(90deg,#ff344c,#f7c84f,#36dce7);box-shadow:0 0 12px #f7c84f}
...
def login_page(names: list[str], message: str = "", selected: str = "") -> str:
    ...
    <section class="card">
      ...
      <details class="section-drawer compact-info">
        ...
        <strong>🥉 Bronze</strong> at 10 purchases — £90 delivery<br>
        <strong>🥈 Silver</strong> at 25 purchases — £75 delivery<br>
        <strong>🥇 Gold</strong> at 50 purchases — +1 City Run sticker and £50 delivery<br>
        <strong>💎 Platinum</strong> at 100 purchases — +1 City Run sticker and £25 delivery<br>
        <strong>👑 SNR VIP</strong> at 200 purchases — +2 City Run stickers and free delivery</p>
...
def membership_gallery(current: str) -> str:
    ...
    for index, (name, level) in enumerate(VIP_LEVELS.items()):
        ...
        tiles.append(f'''<article class="tier-tile {'tier-current' if active else ''}">{art}<h3>{html.escape(name)} {'— Your level' if active else ''}</h3><p>{level["minimum_sales"]}+ purchases</p><ul><li>{level["bonus_points"]} extra City Run sticker(s) per meal deal</li><li>{fee}</li></ul></article>''')
...
def customer_page(customer: dict, claims: ClaimStore, orders: DeliveryStore, shifts: StaffShifts,
    ...
    return page(f'{customer["display_name"]} • SNR City Run', f'''<section class="card quick-hub" id="customer-app"{sync_data}>{top_order}{review_popup}<div id="service-toast" role="status"></div>
      ...
      <div class="app-view active" data-app-view="home" role="tabpanel">{recovery}
        ...
        <button class="neon-reward-banner" type="button" data-tab-target="city-run" aria-label="Open your City Run collection"><span aria-hidden="true">🏁</span><strong>{city_unique}<small>OF 38 FOUND</small></strong><span>YOUR COLLECTION<small>{str(city_reveals) + ' sticker reveal(s) ready — open now' if city_reveals else 'View your SNR City Run board'}</small></span><b>›</b></button>
      ...
      <div class="app-view" data-app-view="visits" role="tabpanel">
        <div class="quick-metrics"><div><small>MEMBERSHIP</small><strong>{membership["emoji"]} {html.escape(membership["name"])}</strong></div><div><small>COLLECTION</small><strong>🏁 {city_unique}/38</strong></div><div><small>VISITS</small><strong>🍔 {int(customer["lifetime_sales"])}</strong></div></div>
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if path == "/city-marketplace.js":
                ...
                self.end_headers()
                self.wfile.write(data)
            elif path in ("/city-run-board.jpg", "/city-run-board-v2.png", "/city-run-board-v3.png", "/city-run-board-v4.png"):
                self.send_response(200)
                self.send_header("Content-Type", "image/png")
