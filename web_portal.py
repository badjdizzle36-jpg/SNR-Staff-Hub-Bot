...
def customer_page(customer: dict, claims: ClaimStore, orders: DeliveryStore, shifts: StaffShifts,
    ...
    if active_order:
        ...
        order_icon = "🛍️" if (active_order.get("fulfillment_type") or "delivery") == "pickup" else "🚗"
    ...
    return page(f'{customer["display_name"]} • SNR Loyalty', f'''<section class="card quick-hub" id="customer-app">{review_popup}
      ...
      <div class="app-view active" data-app-view="home" role="tabpanel">{recovery}
        ...
        <div class="quick-actions"><button type="button" class="quick-action quick-action-main" data-tab-target="order" aria-selected="false"><span>🍔</span><strong>Start New Order</strong><small>Delivery or pickup</small></button><button type="button" class="quick-action" data-tab-target="raffle" aria-selected="false"><span>🎟️</span><strong>Live Raffle</strong><small>Choose numbers</small></button><button type="button" class="quick-action" data-tab-target="rewards" aria-selected="false"><span>🎁</span><strong>My Rewards</strong><small>View and claim</small></button></div>
